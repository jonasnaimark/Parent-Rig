...
function applyExpressions(child, parent, comp, is3D, splitDims, groupBounds, existingRestPos, followOptions) {
    ...
    if (usePseudoEffect) {
        ...
        header = header.concat([
            ...
            '}',
            '',
            'function getMirrorSign(pos) {',
            '    // Returns 1 or -1 based on which side of affector center the item is',
            '    if (!affector) return 1;',
            '    var mirror = 0;',
            '    try { mirror = affector.effect("Mirror Rotation")("Checkbox").value; } catch(e) {}',
            '    if (!mirror) return 1;',
            '    ',
            '    var affectorPos = affector.transform.position.value;',
            '    var dx = pos[0] - affectorPos[0];',
            '    var dy = pos[1] - affectorPos[1];',
            '    ',
            '    // Use affector rotation to determine mirror axis',
            '    // Rotation 0 = horizontal layout (mirror on X), Rotation 90 = vertical layout (mirror on Y)',
            '    var angle = (affector.threeDLayer ? affector.transform.zRotation.value : affector.transform.rotation.value) * Math.PI / 180;',
            ...
            '    return amount * influence * getMirrorSign(pos);',
            '}',
            '',
