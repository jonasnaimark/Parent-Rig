...
function applyExpressions(child, parent, comp, is3D, splitDims, groupBounds, existingRestPos, followOptions) {
    ...
    // Extract dynamic system flags
    var includeAffector = followOptions.includeAffector || false;
    var includeTarget = followOptions.includeTarget || false;
    // Pin Edges and Pivot Transform are now always enabled (small overhead, commonly used)
    var includePinEdges = true;
    var includePivotTransform = true;
    ...
    if (usePseudoEffect) {
        ...
        if (includeAffector) {
            // Add stub definitions for features that affector code references but may not be enabled
            if (!includePivotTransform) {
                header.push('var rotateAroundMode = 1; // Stub: pivot transform disabled');
            }
            if (!includePinEdges) {
                header.push('function getPinEdgeState(t, p) { return {active: false, offsetX: 0, offsetY: 0, influence: 1}; }');
            }
            header = header.concat([
            '// ===== AFFECTOR SYSTEM =====',
            'var affectors = [];',
        ...
        // Conditionally add PIN EDGES SYSTEM (only if checkbox enabled)
        if (includePinEdges) {
            header = header.concat([
            '',
            ...
            '    }',
            '    ',
            '    var pinDirection = pinDirectionProp.valueAtTime(t);',
            ...
            '    ',
            '    // Determine which pin is active',
            '    var activePinEdge = null;',
            '    var pinnedIdx = 0;',
            '    var offset = 0;',
            '    var isVertical = true;',
            '    ',
            ...
            '    ',
            '    // Calculate influence for this layer',
        ...
        header = [
            ...
            '// Pin edges and pivot transform removed for performance testing',
    ...
    // Conditionally add pivot transform code
    if (includePivotTransform) {
        posExpr += '\n' + [
            ...
            '    ',
            '    // For Leader mode, get leader REST position',
            ...
            '                if (childEff) return [childEff(3).value, childEff(4).value' + (is3D ? ', childEff(5).value' : '') + '];',
            '            } catch(e) {}',
            '            try {',
            '                return [leaderLayer.effect("PR_RestPosX")("Slider").value, leaderLayer.effect("PR_RestPosY")("Slider").value' + (is3D ? ', 0' : '') + '];',
            '            } catch(e) {}',
            '            return null;',
            '        }',
            ...
            '            }',
            '        }',
            '        var currentOffset = [offsetFromPivot[0] * (1 + scaleRatioDelta[0]), offsetFromPivot[1] * (1 + scaleRatioDelta[1])' + (is3D ? ', (offsetFromPivot[2] || 0) * (1 + (scaleRatioDelta[2] || 0))' : '') + '];',
            '        var scaleDelta = mul(sub(currentOffset, offsetFromPivot), childInfluence);',
            ...
            '        var cosR = Math.cos(rad);',
            '        var sinR = Math.sin(rad);',
            '        var rx = offsetFromPivot[0] * cosR - offsetFromPivot[1] * sinR;',
            '        var ry = offsetFromPivot[0] * sinR + offsetFromPivot[1] * cosR;',
            '        var rotatedOffset = [rx, ry' + (is3D ? ', offsetFromPivot[2] || 0' : '') + '];',
            '        var rotateDelta = mul(sub(rotatedOffset, offsetFromPivot), childInfluence);',
    ...
    // Conditionally add pin edges code
    if (includePinEdges) {
        posExpr += '\n' + [
    ...
    var rotExpr = header + timeRemapFunc + [
        ...
        'var rotProp = parentLayer.transform.rotation;',
        '',
        '// Normal rotation following - uses accumulated delta',
    ...
    if (followOptions.position) {
        if (splitDims) {
            ...
            var xPosExpr = createSplitPosExpr(header, timeRemapFunc, 'X', 'xPosition', is3D, includeAffector, includeTarget, includePinEdges, includePivotTransform);
            var yPosExpr = createSplitPosExpr(header, timeRemapFunc, 'Y', 'yPosition', is3D, includeAffector, includeTarget, includePinEdges, includePivotTransform);

            try { child.transform.xPosition.expression = xPosExpr; } catch (e) {}
            try { child.transform.yPosition.expression = yPosExpr; } catch (e) {}

            if (is3D) {
                var zPosExpr = createSplitPosExpr(header, timeRemapFunc, 'Z', 'zPosition', is3D, includeAffector, includeTarget, includePinEdges, includePivotTransform);
...
function createSplitPosExpr(header, timeRemapFunc, axis, propName, is3D, includeAffector, includeTarget, includePinEdges, includePivotTransform) {
    ...
    // Conditionally add pivot transform code for split dimensions
    if (includePivotTransform) {
        expr += '\n' + [
            '// Pivot transform for split dimensions (simplified - no 3D rotation support)',
            'if (scaleAroundMode > 1 || rotateAroundMode > 1) {',
            '    var restPosX = cp("Rest Pos X");',
            '    var restPosY = cp("Rest Pos Y");',
            '    var parentRestPosX = cp("Parent Rest Pos X");',
            ...
            '        var leaderIdx = getLeaderIndexAtTime(time);',
            '        if (scaleAroundMode === 3) {',
            '            if (myIndex === leaderIdx) skipScale = true;',
            '            else {',
            '                var ldr = findLeaderLayer(time);',
            '                if (ldr) try { scalePivotX = ldr.effect("Parent Rig - Child")(3).value; scalePivotY = ldr.effect("Parent Rig - Child")(4).value; } catch(e) {}',
            '            }',
            '        }',
            '        if (rotateAroundMode === 3) {',
            '            if (myIndex === leaderIdx) skipRotate = true;',
            '            else {',
            '                var ldr = findLeaderLayer(time);',
            '                if (ldr) try { rotatePivotX = ldr.effect("Parent Rig - Child")(3).value; rotatePivotY = ldr.effect("Parent Rig - Child")(4).value; } catch(e) {}',
            '            }',
            '        }',
            '    }',
            '    var offsetX = restPosX - scalePivotX;',
            '    var offsetY = restPosY - scalePivotY;',
            '    var rotOffsetX = restPosX - rotatePivotX;',
            '    var rotOffsetY = restPosY - rotatePivotY;',
            '    if (scaleAroundMode > 1 && !skipScale) {',
            '        var scaleProp = parentLayer.transform.scale;',
            '        var scaleRemapInfo = getRemapInfo(time, scaleProp);',
            '        var parentScaleT = scaleProp.valueAtTime(scaleRemapInfo.t1);',
            '        var scaleRatioX = ((parentScaleT[0] / cp("Parent Rest Scale X")) - 1) * scaleRemapInfo.segInfluence;',
            '        var scaleRatioY = ((parentScaleT[1] / cp("Parent Rest Scale Y")) - 1) * scaleRemapInfo.segInfluence;',
            '        offsetX *= (1 + scaleRatioX);',
            '        offsetY *= (1 + scaleRatioY);',
            '    }',
            '    if (rotateAroundMode > 1 && !skipRotate) {',
            '        var rotProp = parentLayer.transform.rotation;',
            '        var rotRemapInfo = getRemapInfo(time, rotProp);',
            '        var parentRotT = rotProp.valueAtTime(rotRemapInfo.t1);',
            '        var rotDelta = (parentRotT - cp("Parent Rest Rotation")) * rotRemapInfo.segInfluence;',
            '        var rad = rotDelta * Math.PI / 180;',
            '        var cosR = Math.cos(rad);',
            '        var sinR = Math.sin(rad);',
            '        var newRotX = rotOffsetX * cosR - rotOffsetY * sinR;',
            '        var newRotY = rotOffsetX * sinR + rotOffsetY * cosR;',
            '        rotOffsetX = newRotX;',
            '        rotOffsetY = newRotY;',
            '    }',
            (axis === 'X' ? '    parentedPos += (offsetX - (restPosX - scalePivotX) + rotOffsetX - (restPosX - rotatePivotX)) * childInfluence;' : ''),
            (axis === 'Y' ? '    parentedPos += (offsetY - (restPosY - scalePivotY) + rotOffsetY - (restPosY - rotatePivotY)) * childInfluence;' : ''),
            '}',
            ''
        ].join('\n');
    }

    // Conditionally add pin edges code for split dimensions
    if (includePinEdges) {
        expr += '\n' + [
            '// Apply Pin Edges offset for ' + axis + ' axis',
            'var pinParentRestPos = [cp("Parent Rest Pos X"), cp("Parent Rest Pos Y")];',
