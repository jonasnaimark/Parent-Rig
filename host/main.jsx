...
function applyExpressions(child, parent, comp, is3D, splitDims, groupBounds) {
    ...
    if (usePseudoEffect) {
        header = [
            ...
            'var delayFalloffProp = pEff(5);',
            '',
            '// Influence section (indices 10-13)',
            'var parentInfluenceProp = pEff(10);',
            'var pinModeProp = pEff(11);',      // 1=None, 2=First, 3=Last
            'var pinInfluenceProp = pEff(12);',
            ...
            'var leaderIndexProp = pEff(25);',
            'var delayBeforeLeaderProp = pEff(26);',
            'var delayAfterLeaderProp = pEff(27);',
            '',
            ...
            '}',
            '',
            '// Calculate Pin influence multiplier for overscroll stretch effect',
            'function getPinInfluenceMultiplier(t) {',
            '    var pinMode = pinModeProp.valueAtTime(t);',  // 1=None, 2=First, 3=Last (use valueAtTime for keyframe support)
            ...
            '',
            '    // Calculate stretch gradient using square root curve',
            '    // This creates biggest gaps near pinned layer, smaller gaps further away',
            ...
            '',
            '    // Gentle curve (power 0.7): between linear and square root',
            '    // Result: bigger gap at pinned layer, smaller gaps toward trim, but not too extreme',
    ...
    var timeRemapFunc = [
        ...
        '    var hasPinKeys = pinModeProp.numKeys > 0 || pinInfluenceProp.numKeys > 0 || pinTrimProp.numKeys > 0;',
        '    var hasPinEffect = pinModeProp.value !== 1 || hasPinKeys;',  // Pin active or has keyframes
        '    ',
        '    // Fast path: constant 0% influence - child ignores parent entirely',
        '    if (!hasInfluenceKeys && !hasPinEffect && currentInfluence === 0) {',
        '        return prop.value.length === 3 ? [0,0,0] : [0,0];',
        '    }',
        '    ',
        '    // Fast path: constant 100% influence (only if no pin effect) - use simple time remapping',
        '    if (!hasInfluenceKeys && !hasPinEffect && currentInfluence === 1) {',
