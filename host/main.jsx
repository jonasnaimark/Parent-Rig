...
function addAffector() {
    ...
    // Check which affectors already exist
    var hasAffector1 = false;
    var hasAffector2 = false;
    for (var i = 1; i <= comp.numLayers; i++) {
        if (comp.layer(i).name === "Parent Rig Affector") hasAffector1 = true;
        if (comp.layer(i).name === "Parent Rig Affector 2") hasAffector2 = true;
    }

    // Determine which affector to create
    var affectorName;
    if (!hasAffector1) {
        affectorName = "Parent Rig Affector";
    } else if (!hasAffector2) {
        affectorName = "Parent Rig Affector 2";
    } else {
        alert("Maximum of 2 affectors allowed per composition.");
        return "exists";
    }
    ...
    try {
        ...
        // Colors: Affector 1 = Orange, Affector 2 = Cyan
        var isSecondAffector = (affectorName === "Parent Rig Affector 2");
        var outerColor = isSecondAffector ? [0, 0.8, 1, 1] : [1, 0.5, 0, 1];      // Cyan or Orange
        var innerColor = isSecondAffector ? [0.3, 0.9, 1, 1] : [1, 0.7, 0.3, 1];  // Light cyan or Light orange
...
function applyExpressions(child, parent, comp, is3D, splitDims, groupBounds, existingRestPos, followOptions) {
    ...
    if (usePseudoEffect) {
        ...
        header = header.concat([
            ...
            'try { affector2 = thisComp.layer("Parent Rig Affector 2"); } catch(e) {}',
            '',
            ...
            '    // Affector 1',
            '    var inf1 = calcInfluenceFor(affector, pos);',
            '    if (inf1 > 0) {',
            '        var px1 = 0, py1 = 0, pz1 = 0;',
            '        try { px1 = affector.effect("Position X")("Slider").value; } catch(e) {}',
            '        try { py1 = affector.effect("Position Y")("Slider").value; } catch(e) {}',
            '        try { pz1 = affector.effect("Position Z")("Slider").value; } catch(e) {}',
            '        totalX += px1 * inf1;',
            '        totalY += py1 * inf1;',
            '        totalZ += pz1 * inf1;',
            '    }',
            '    // Affector 2',
            '    var inf2 = calcInfluenceFor(affector2, pos);',
            '    if (inf2 > 0) {',
            '        var px2 = 0, py2 = 0, pz2 = 0;',
            '        try { px2 = affector2.effect("Position X")("Slider").value; } catch(e) {}',
            '        try { py2 = affector2.effect("Position Y")("Slider").value; } catch(e) {}',
            '        try { pz2 = affector2.effect("Position Z")("Slider").value; } catch(e) {}',
            '        totalX += px2 * inf2;',
            '        totalY += py2 * inf2;',
            '        totalZ += pz2 * inf2;',
            '    }',
            ...
            '    var total = 0; // accumulate deltas from 100',
            '    // Affector 1',
            '    var inf1 = calcInfluenceFor(affector, pos);',
            '    if (inf1 > 0) {',
            '        var amt1 = 100;',
            '        try { amt1 = affector.effect("Scale")("Slider").value; } catch(e) {}',
            '        total += (amt1 - 100) * inf1;',
            '    }',
            '    // Affector 2',
            '    var inf2 = calcInfluenceFor(affector2, pos);',
            '    if (inf2 > 0) {',
            '        var amt2 = 100;',
            '        try { amt2 = affector2.effect("Scale")("Slider").value; } catch(e) {}',
            '        total += (amt2 - 100) * inf2;',
            '    }',
            '    return 100 + total;',
            '}',
            ...
            '    // Affector 1',
            '    var mirror1 = 0;',
            '    try { mirror1 = affector.effect("Mirror Rotation")("Checkbox").value; } catch(e) {}',
            '    var inf1 = mirror1 ? calcSpatialInfluenceFor(affector, pos) : calcInfluenceFor(affector, pos);',
            '    if (inf1 > 0) {',
            '        var amt1 = 0;',
            '        try { amt1 = affector.effect("Rotation X")("Slider").value; } catch(e) {}',
            '        total += amt1 * inf1 * getMirrorSignFor(affector, pos);',
            '    }',
            '    // Affector 2',
            '    var mirror2 = 0;',
            '    try { mirror2 = affector2.effect("Mirror Rotation")("Checkbox").value; } catch(e) {}',
            '    var inf2 = mirror2 ? calcSpatialInfluenceFor(affector2, pos) : calcInfluenceFor(affector2, pos);',
            '    if (inf2 > 0) {',
            '        var amt2 = 0;',
            '        try { amt2 = affector2.effect("Rotation X")("Slider").value; } catch(e) {}',
            '        total += amt2 * inf2 * getMirrorSignFor(affector2, pos);',
            '    }',
            '    return total;',
            '}',
            '',
            ...
            '    if (inf1 > 0) {',
            '        var amt1 = 0;',
            '        try { amt1 = affector.effect("Rotation Y")("Slider").value; } catch(e) {}',
            '        total += amt1 * inf1 * getMirrorSignFor(affector, pos);',
            '    }',
            '    // Affector 2',
            '    var mirror2 = 0;',
            ...
            '    if (inf2 > 0) {',
            '        var amt2 = 0;',
            '        try { amt2 = affector2.effect("Rotation Y")("Slider").value; } catch(e) {}',
            '        total += amt2 * inf2 * getMirrorSignFor(affector2, pos);',
            '    }',
            '    return total;',
            '}',
            ...
            '    if (inf1 > 0) {',
            '        var amt1 = 0;',
            '        try { amt1 = affector.effect("Rotation Z")("Slider").value; } catch(e) {}',
            '        total += amt1 * inf1 * getMirrorSignFor(affector, pos);',
            '    }',
            '    // Affector 2',
            '    var mirror2 = 0;',
            ...
            '    if (inf2 > 0) {',
            '        var amt2 = 0;',
            '        try { amt2 = affector2.effect("Rotation Z")("Slider").value; } catch(e) {}',
            '        total += amt2 * inf2 * getMirrorSignFor(affector2, pos);',
            '    }',
            '    return total;',
            '}',
            ...
            '    if (inf1 > 0) {',
            '        var amt1 = 100;',
            '        try { amt1 = affector.effect("Opacity")("Slider").value; } catch(e) {}',
            '        total += (amt1 - 100) * inf1;',
            '    }',
            '    // Affector 2',
            '    var inf2 = calcInfluenceFor(affector2, pos);',
            '    if (inf2 > 0) {',
            '        var amt2 = 100;',
            '        try { amt2 = affector2.effect("Opacity")("Slider").value; } catch(e) {}',
            '        total += (amt2 - 100) * inf2;',
            '    }',
            '    return 100 + total;',
            '}',
